<template>
    <!--get-container 设置body为根节点
    lazy-render（是否显示弹层时才渲染节点）关闭懒加载 -->
  <van-popup
    :value="value"
...
<script>
import { getAllChannels, deleteUserChannel, updataUserChannel } from '@/api/channel'
export default {
  name: 'HomeChannel',
  props: {
    value: {
  ...
  methods: {
    ...
    async handleUserChannelClick (item, index) {
      ...
      // 如果是编辑状态，则是删除操作
      const channels = this.userChannels.slice(0) // 复制数组
